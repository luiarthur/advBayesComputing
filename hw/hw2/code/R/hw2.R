...
plot.coverage <- function(b_post,b_true=beta,pp=6,...) {
  ci <- t(apply(b_post,2,function(x) quantile(x,c(.025,.975))))
  M <- cbind(b_true[1:pp],ci[1:pp,]); M <- cbind(M,t(apply(M,1,function(x) c( diff(c(x[2],x[3])), x[1] %bw% c(x[2],x[3])) )))
  colnames(M) <- c("Truth","CI lower","CI upper","Length","Coverage")
  rng <- range(M[,1:3])

  plot(M[,1],ylim=rng,bty="n",xaxt="n",...)
  axis(1,at=1:pp,label=paste0("b",1:pp))
  add.errbar(ci,lwd=4,col="blue")
}

# Tables & Plots
ci <- t(apply(b.post,2,function(x) quantile(x,c(.025,.975))))
M <- cbind(beta,ci); M <- cbind(M,t(apply(M,1,function(x) c( diff(c(x[2],x[3])), x[1] %bw% c(x[2],x[3])) )))
colnames(M) <- c("Truth","CI lower","CI upper","Length","Coverage"); M
pdf("output/probit.pdf",w=13,h=9)
  plot.coverage(b.post,pch=20,col="grey",cex=3,ylab="",xlab=expression(beta),pp=20)
dev.off()

mean(M[-c(1:6),"Coverage"])
which(beta == 0 & beta %bw% M[,2:3])
