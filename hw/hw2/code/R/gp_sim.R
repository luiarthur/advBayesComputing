...
# y | ... ~ N(0,s^2 + K)
prelim <- gp(y, x, s, C, D, cand_S=diag(3), init=rep(0,3), B=5000, burn=5000, printProg=T)
...
system.time( preds <- t(apply(out$param[1:1000,],1,function(p) onePred(p,out))) )
...
par(mfrow=c(1,2))
  ...
  plotmap(apply(preds,2,mean),x,xlim=c(-2,2),ylim=c(-3,3),col.map=col.map)
par(mfrow=c(1,1))

system.time( preds_ms <- t(apply(out$param[1:1000,],1,function(p) onePred_mu_star(p,out))) )
par(mfrow=c(1,2))
  plotmap(f(s),s,bks=c(0,3),xlim=c(-2,2),ylim=c(-3,3),col.map=col.map.s)
  plotmap(apply(preds_ms,2,mean),s,bks=c(0,3),xlim=c(-2,2),ylim=c(-3,3),col.map=col.map.s)
par(mfrow=c(1,1))
